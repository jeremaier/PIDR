...
public class LameHistologiqueDaoImpl extends DaoAutoIncrementImpl implements LameHistologiqueDao {
    ...
    private HistologicLamella addToLame(ResultSet resultSet) throws SQLException {
        ...
        lame.setNumLame(resultSet.getString("NUM_LAME"));
    ...
    public void update(HistologicLamella lame, int id) {
        ...
        try {
            ...
            System.out.println("UPDATE lame SET ID_LESION=?,NUM_LAME=?,  SITE_COUPE = ?, ORIENTATION_NOIR = ?, ORIENTATION_VERT = ?, COLORATION = ?, PHOTO = ? WHERE ID = ?");
        } catch (MySQLNonTransientConnectionException e) {
            FileManager.openAlert("La connection avec le serveur est interrompue");
            e.printStackTrace();
        } catch (Exception e) {
